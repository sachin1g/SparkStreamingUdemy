...
object SessionizerPractice {
   ...
   */
  case class SessionData(val sessionLength: Long, var statusValue:List[String],var statusCount:List[Int]);
   ...
   */
  def trackStateFunc(batchTime: Time, ip: String, data: Option[(String,Int)], state: State[SessionData]): Option[(String, SessionData)] = {
    // Extract the previous state passed in (using getOrElse to handle exceptions)
    val previousState = state.getOption.getOrElse(SessionData(0, List(),List()))
    
    // Create a new state that increments the session length by one, adds this URL to the clickstream, and clamps the clickstream 
    // list to 10 items
    val newState = SessionData(previousState.sessionLength + 1L, previousState.statusValue :+ data.getOrElse(("empty",0))._1,previousState.statusCount :+ data.getOrElse(("empty",0))._2)
  ...
  def codeToStatus(status : String):String={
  ...
  def main(args: Array[String]) {
    ...
    val requests = lines.map(x => {
      ...
      if (matcher.matches()) {
        ...
        (ip+"_"+codeToStatus(status),1)
      } else {
        ("error_error",1)
      }
    })
      val statusCountByIP=requests.transform(rdd=>rdd.reduceByKey((x,y)=>x+y)
    ...
    val requestsWithState = statusCountByIP.mapWithState(stateSpec)
    ...
    stateSnapshotStream.foreachRDD((rdd, time) => {
      ...
      val requestsDataFrame = rdd.map(x => (x._1, x._2.sessionLength, x._2.statusValue,x._2.statusCount)).toDF("ip", "sessionLength", "statusValue","statusCount")
